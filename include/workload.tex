...
our work since, as motivated in \sref{motivation}, detailed simulations are too
time consuming. Instead, we propose the use of high-level recordings; see the
modules labeled ``Recorder'' in \fref{methodology}. To elaborate, using an
...
module labeled ``Streamer'' in \fref{methodology}), the collected information is
...
will be further elaborated on in the next subsection, \sref{composition}.
...
or opt for tracking \sc{ALU}, \sc{FPU}, and so on individually. The resource
